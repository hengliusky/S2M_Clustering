...
class ReDGAE(nn.Module):
    ...
    def train(self, adj_add_norm, features_add, adj_add, adj_add_label,weight_tensor_add, norm_add,edge_index_add,
        ...
        self.load_state_dict(torch.load(save_path + dataset + '/pretrain/my_model_w_f.pk'), False)
        if optimizer == "Adam":
        ...
        unconflicted_ind_rm = []
        conflicted_ind_rm = []
        print(self.assignment_add.cluster_centers,self.assignment_rm.cluster_centers)
        for epoch in epoch_bar:
            opti.zero_grad()
            emb_add_o = self.encode_add(features_add, adj_add_norm)
            #t_sne( emb_add_o.detach().numpy(), y, 19717, True)
            ...
            if epoch % 5 == 0:
                unconflicted_ind_add, conflicted_ind_add = generate_unconflicted_data_index(emb_add.detach().numpy(),
                                                                                    ...
                                                                                    beta1_add, beta2_add)
                unconflicted_ind_rm, unconflicted_ind_rm = generate_unconflicted_data_index(emb_rm.detach().numpy(),
                                                                                    self.assignment_rm.cluster_centers.detach().numpy(),
                                                                                    beta1_rm, beta2_rm)
            # if epoch == 0:
            #     adj, adj_label, weight_tensor = self.update_graph(adj, y, emb, unconflicted_ind)
            if len(previous_unconflicted_add) < len(unconflicted_ind_add):
                emb_add_unconf = emb_add[unconflicted_ind_add]
                p_add_unconf = p_add[unconflicted_ind_add]
                q_add_unconf = q_add[unconflicted_ind_add]
                previous_conflicted_add = conflicted_ind_add
                previous_unconflicted_add = unconflicted_ind_add
            else:
                epoch_add_stable += 1
                emb_add_unconf = emb_add[previous_unconflicted_add]
                p_add_unconf = p_add[previous_unconflicted_add]
                q_add_unconf = q_add[previous_unconflicted_add]
            if epoch_add_stable >= 15:
                epoch_add_stable = 0
                beta1_add = beta1_add * 0.95
                beta2_add = beta2_add * 0.85

            if len(previous_unconflicted_rm) < len(unconflicted_ind_rm):
                emb_rm_unconf = emb_rm[unconflicted_ind_rm]
                p_rm_unconf = p_rm[unconflicted_ind_rm]
                q_rm_unconf = q_rm[unconflicted_ind_rm]
                previous_conflicted_rm= conflicted_ind_rm
                previous_unconflicted_rm = unconflicted_ind_rm
            else:
                epoch_rm_stable += 1
                emb_rm_unconf = emb_rm[previous_unconflicted_rm]
                p_rm_unconf = p_rm[previous_unconflicted_rm]
                q_rm_unconf = q_rm[previous_unconflicted_rm]
            ...
            if epoch % 50 == 0 and epoch <= 200:
                adj_add, adj_add_label, weight_tensor_add = self.update_graph(adj_add, y, emb_add, unconflicted_ind_add)
                edge_index_add = torch.tensor(np.vstack((adj_add_label._indices()[0], adj_add_label._indices()[1])),
                                              dtype=torch.int32)
                adj_rm, adj_rm_label, weight_tensor_rm = self.update_graph(adj_rm, y, emb_rm, unconflicted_ind_rm)
                edge_index_rm = torch.tensor(np.vstack((adj_rm_label._indices()[0], adj_rm_label._indices()[1])),
                                             dtype=torch.int32)
            loss, _, _ = self.loss(q_add_unconf, p_add_unconf, x_add, adj_add_label, weight_tensor_add, cache_add,
                                   q_rm_unconf, p_rm_unconf, x_rm, adj_rm_label, weight_tensor_rm, cache_rm)
            ...
            if epoch % 50 == 0 and epoch <= 200 and epoch != 0:
            ...
            lr_s.step()
            if epoch == 199:
                t_sne(emb_add.detach().numpy(), y, 19717, True)
