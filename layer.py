...
class SuperGAT(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, heads,dropout,concat,attention_type,
                 neg_sample_ratio, edge_sample_ratio, **kwargs):
        ...
        self.is_super_gat = True
        self.to_undirected_at_neg = False
        if self.attention_type == "gat_originated":  # GO
            ...
            self.scaling_factor = np.sqrt(self.out_channels)
    ...
    def forward(self, x, edge_index, size=None,batch=None,
        ...
        :return:
        """
        #edge_index_add [2, 16572]
        if size is None and torch.is_tensor(x):
            device = "cpu"
            edge_index, _ = remove_self_loops(edge_index)
            edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0))
        # self-loop edge_index_add [2, 19280]
        ...
        if self.is_super_gat:
            ...
            if self.edge_sample_ratio < 1.0:
                ...
                pos_indices = torch.tensor(pos_indices).long()
            ...
            # Labels
            # if self.cache["att_label"] is None:
    ...
    @staticmethod
    def get_supervised_attention_loss(cache):
